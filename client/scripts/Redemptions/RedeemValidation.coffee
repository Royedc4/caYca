...
angular.module('app.redemptions.validation', [])

# Roy: When Finish writting maxLenght
.directive('ngRedeemTokensTabs',  ->
	return { 
		link: (scope, element, attrs) ->
			element.bind('keypress', (event) ->
				...
				if (tokenLiveLength>=12)
					if parseInt(this.name.substr(5,3))>=scope.forms.quantity
						document.getElementById('registerToken4redeemButton').focus()
